	...
	, SB_TYPE _Type, bool _bSysAccess, void* _pSysMem)
{
	...
	// 상수버퍼 생성
	m_desc.ByteWidth = iBufferSize;				// 버퍼 크기
	m_desc.StructureByteStride = m_elementSize;	// 데이터 간격

	if (SB_TYPE::READ_ONLY == m_SBType)
	{
		m_desc.BindFlags = D3D11_BIND_SHADER_RESOURCE;	// Texture 레지스터에 바이딩하기 위한 플래그
	}
	...
	m_desc.MiscFlags = D3D11_RESOURCE_MISC_BUFFER_STRUCTURED;	// 구조화 버퍼 체크
