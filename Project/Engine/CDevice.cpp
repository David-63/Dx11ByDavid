...
{
    m_renderResolution = Vec2(static_cast<float>(_iWidth), static_cast<float>(_iHeight));
    g_globalData.Resolution = m_renderResolution;
    ...
    // ViewPort 설정
    CD3D11_VIEWPORT viewport(0.0f, 0.0f, static_cast<FLOAT>(_iWidth), static_cast<FLOAT>(_iHeight));
...
int CDevice::CreateSwapChain()
{
    ...
    HWND windo = CEngine::GetInst()->GetMainWnd();
    tDesc.OutputWindow = windo;                             // 출력 윈도우
    tDesc.Windowed = true;                                  // 창모드, 전체화면 모드

    tDesc.BufferCount = 1;
    tDesc.BufferUsage = DXGI_USAGE_RENDER_TARGET_OUTPUT;
    tDesc.BufferDesc.Width = static_cast<UINT>(m_renderResolution.x);
    tDesc.BufferDesc.Height = static_cast<UINT>(m_renderResolution.y);
    tDesc.BufferDesc.Format = DXGI_FORMAT::DXGI_FORMAT_R8G8B8A8_UNORM;
    tDesc.BufferDesc.RefreshRate.Denominator = 1;           // 0 : 프레임 제한 off,      1 : 프레임 제한 on
    tDesc.BufferDesc.RefreshRate.Numerator = 120;
...
int CDevice::CreateView()
{
    ...
    m_DSTex = CResMgr::GetInst()->CreateTexture(L"DepthStencilTex"
        , static_cast<UINT>(m_renderResolution.x), static_cast<UINT>(m_renderResolution.y)
...
{
    m_arrConstBuffer[(UINT)CB_TYPE::TRANSFORM] = new CConstantBuffer((UINT)CB_TYPE::TRANSFORM);
    m_arrConstBuffer[(UINT)CB_TYPE::TRANSFORM]->Create(sizeof(tTransform), 1);

    m_arrConstBuffer[(UINT)CB_TYPE::MATERIAL] = new CConstantBuffer((UINT)CB_TYPE::MATERIAL);
    m_arrConstBuffer[(UINT)CB_TYPE::MATERIAL]->Create(sizeof(tMtrlConst), 1);

    m_arrConstBuffer[(UINT)CB_TYPE::GLOBAL] = new CConstantBuffer((UINT)CB_TYPE::GLOBAL);
